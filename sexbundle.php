...
define('sexbundle_plugin_version', '1.3');
...
function sb_ST4_columns_content($column_name, $post_ID) {
    if ($column_name == 'sexbundle') {
    	global $wpdb;
  			$sb_article_url = end($wpdb->get_results( 'SELECT *  FROM '.$wpdb->prefix.'sexbundle_connections  WHERE ID_post =  '.$post_ID.'  LIMIT 1'));
...
if(isset($_POST['sb_update_account'])){
	...
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS,
	                "action=check_secret_key&sb_username=".$_POST['sb_username'].'&sb_password='.$_POST['sb_password']  
	            );
	...
	if ($answer->error == "0") { 
		...
		update_option( 'sb_username', $_POST['sb_username'] );
		update_option( 'sb_password', $_POST['sb_password'] );
	} 
		else { 
...
if( isset($_POST['sb_select_web'])):
	...
	update_option( 'sb_sweb', $_POST['sb_sweb'] );

	$json = array();
	$json['sb_sweb'] =  $_POST['sb_sweb'] ;
	$json['sb_skey'] =  $sk; ;
...
if(isset($_GET['post'])){
			...
			$html .= '<div id="sb_overlay" style="position:fixed; z-index:999; display:none; height:2500px; width:2500px; left:0px; top:0px; background-image:url('.plugins_url('sexbundle/transparent_bg.png').');"></div>';
			$html .= '<div style="width:600px; display:none;  position:fixed; z-index:9999; left:50%; padding:10px; margin-left:-300px; top:20%; background-color:#fff; border:1px solid #ccc" id="submit_to_sb_form">
						...
						<div style="float:left">
							<div id="sb_thumb_wrapper">'.
								get_the_post_thumbnail($_GET['post'], 'sb_thumb', array('id' => 'article_thumb_image','style'=>' width:200px; height:auto; min-height:150px; display:block'));
								if(strlen($url_img) <= 2){
					...
					$html .='
						...
						<div style="float:left; width:387px; margin-left:10px;">
							<p style="margin-top:0px;"><label>Article URL</label><br/><input name="sb_article_url" id="sb_article_url" placeholder="Article URL" type="text" style="width:100%;border:1px solid #cccccc" value="'.get_permalink($_POST['post']).'"></p>
							...
							<p><label>Article short description</label><br/><textarea  style="width:100%;height:130px;border:1px solid #cccccc"  placeholder="Article description" name="sb_article_title_content" id="sb_article_title_content">'.sb_get_the_excerpt($_GET['post']).'</textarea></p>
						</div>
...
function sb_custom_button(){
    	...
    	if(isset($_GET['post'])){
			...
			$sb_article_url = end($wpdb->get_results( 'SELECT *  FROM '.$wpdb->prefix.'sexbundle_connections  WHERE ID_post =  '.$_GET['post'].'  LIMIT 1'));
